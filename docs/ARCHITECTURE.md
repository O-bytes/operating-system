...
> **Note on current implementation:** See [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) for a precise mapping of which parts of this architecture are fully wired and tested today versus which remain skeletal. The core reactive loop (watcher → dispatcher → subsystems → effector) is live; many advanced subsystem behaviors are still being filled in.
