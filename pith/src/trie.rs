...
impl TrieNode {
    ...
    /// Collect the full paths of all leaf descendants, relative to this node.
    /// Example: a subtree holding events/!foo and tmp/a/b yields
    /// ["events/!foo", "tmp/a/b"] — intermediate directories are not included.
    pub fn leaf_paths(&self) -> Vec<String> {
        fn walk(prefix: &str, node: &TrieNode, out: &mut Vec<String>) {
...
impl Trie {
    ...
    pub fn build(fs_root: &Path, alphabet: &Alphabet) -> Result<Self> {
        ...
        for entry in walker {
            ...
            // Exception: password hashes are stored as `.argon2id.*` filenames
            // under -secret/ and must be indexed for auth to work.
            if relative.components().any(|c| {
                let name = c.as_os_str().to_string_lossy();
                name.starts_with('.') && !name.starts_with(".argon2id.")
            }) {
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn test_secret_hashes_indexed_despite_leading_dot() {
        let (dir, alphabet) = setup_test_fs();

        // Password hashes live in filenames that start with a dot.
        let secret = dir.path().join("hard/identities/001/-secret");
        std::fs::create_dir_all(&secret).unwrap();
        let hash_name = ".argon2id.v=19.m=19456,t=2,p=1.SALT.DIGEST";
        std::fs::File::create(secret.join(hash_name)).unwrap();

        let trie = Trie::build(dir.path(), &alphabet).unwrap();

        let node = trie.get(&["hard", "identities", "001", "-secret", hash_name]);
        assert!(node.is_some(), "the .argon2id hash must be indexed for auth");
    }
}
