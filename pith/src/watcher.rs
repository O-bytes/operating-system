...
impl FsWatcher {
    ...
    pub fn poll(&self, timeout: Duration) -> Option<WatchEvent> {
        loop {
            ...
            // Skip dotfiles (except `.argon2id.*` password hashes, which must be indexed).
            if let Some(name) = abs_path.file_name() {
                let name = name.to_string_lossy();
                if name.starts_with('.') && !name.starts_with(".argon2id.") {
            ...
            // Loop avoidance: drop events caused by the engine's own writes.
            // The trie was already updated synchronously by the effector.
            if self.effector.is_engine_event(&abs_path) {
                trace!(
...
mod tests {
    ...
    async fn test_watcher_skips_effector_events() {
        ...
        // The watcher should NOT report this (recent-writes filter).
        let event = watcher.poll(Duration::from_millis(500));
        assert!(
            event.is_none(),
