...
use crate::subsystems::scheduler::{self, SchedulerSubsystem};
use crate::subsystems::programs::{self, ProgramsSubsystem};
...
pub async fn boot(config: &PithConfig) -> Result<PithEngine> {
    ...
    // 7. Create effector and attach the trie: every successful effect
    // is indexed synchronously (the watcher drops engine-generated events).
    let effector = Effector::new(fs_root.clone());
    effector.attach_index(Arc::clone(&trie), alphabet.clone());
    ...
    subsystems.register(Box::new(EventsSubsystem::new_with_trie(Arc::clone(&trie))));
    ...
    subsystems.register(Box::new(WorkersSubsystem::new_with_trie(Arc::clone(&trie))));
    subsystems.register(Box::new(SchedulerSubsystem::new()));
    subsystems.register(Box::new(ProgramsSubsystem::new_with_trie(Arc::clone(&trie))));
...
pub async fn run(engine: &PithEngine) -> Result<()> {
    ...
    let scheduler_interval = Duration::from_millis(900);
    let mut last_scheduler_tick = Instant::now();
    let program_reeval_interval = Duration::from_millis(1200);
    let mut last_program_reeval = Instant::now();
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep(poll_interval) => {
                ...
                if last_scheduler_tick.elapsed() >= scheduler_interval {
                    ...
                    if fired > 0 {
                        event_count += fired as u64;
                        debug!("Scheduler tick fired {} task(s)", fired);
                    }
                    last_scheduler_tick = Instant::now();
                }

                // Reactive program transitions — programs keep advancing
                // even after the initial !run burst.
                if last_program_reeval.elapsed() >= program_reeval_interval {
                    last_program_reeval = Instant::now();
                    let prog_effects = programs::reevaluate_programs(&engine.trie);
                    for effect in &prog_effects {
                        if let Err(e) = engine.effector.execute(effect).await {
                            tracing::error!("Program reevaluation effect failed: {}", e);
                        }
                    }
                    if !prog_effects.is_empty() {
                        debug!("Program reactive transitions advanced {} steps", prog_effects.len());
                    }
                }
            }
        }
